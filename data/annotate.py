...
def encode_and_pad(response):
    enc = encoder.get_encoder('gpt-2/models/124M')
...
def annotate(inputfile):
    ...
    decoded_input_path = input_path.split(
        'datasets/')[0] + 'datasets/decoded_' + input_path.split('datasets/')[1]
    ...
    output_path = input_path.split('.json')[0] + '_' + name + ".json"
    decoded_output_path = output_path.split(
        'datasets/')[0] + 'datasets/decoded_' + output_path.split('datasets/')[1]
